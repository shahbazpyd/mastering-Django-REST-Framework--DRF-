...
#     async def notification(self, event):
#         # 'notification' must match "type" from group_send (minus the dots)
...
class NotificationConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        # Always define a default group_name, even if you later close
        self.group_name = None

        # If you want auth: reject anonymous users
        if user.is_anonymous:
            await self.close()
            return

        # For authenticated users, join their group
        self.group_name = f"user_{user.id}"
    ...
    async def disconnect(self, close_code):
        # Only discard if group_name was set
        if self.group_name is not None:
            await self.channel_layer.group_discard(self.group_name, self.channel_name)

    async def notification(self, event):
        # event["type"] == "notification" from group_send
        payload = event.get("payload", {})
        await self.send(text_data=json.dumps(payload))
